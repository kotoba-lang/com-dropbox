(ns dropbox.main
  "Kotodama WASM entrypoint for the Dropbox clean-room actor (L4) — Clojure port.
  ...
  facts (`dropbox.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dropbox")
(def tier "L4")
...
(def entity-specs
  [{:entity "FileMetadata"   :plural "filemetadatas"   :id-prefix "dropbox_fil"
    :fields [:name :pathLower :pathDisplay :rev :size :isDownloadable :contentHash] :required [:name :pathLower]
    :coerce {:size :int :isDownloadable :bool} :refs {}}
   {:entity "FolderMetadata"  :plural "foldermetadatas"  :id-prefix "dropbox_fol"
    :fields [:name :pathLower :pathDisplay :sharedFolderId] :required [:name :pathLower]
    :coerce {} :refs {:sharedFolderId "SharedFolder"}}
   {:entity "SharedLink"      :plural "sharedlinks"      :id-prefix "dropbox_sha"
    :fields [:url :path :visibility :requestedVisibility] :required [:url :path]
    :coerce {} :refs {}}
   {:entity "Account"         :plural "accounts"         :id-prefix "dropbox_acc"
    :fields [:accountId :email :emailVerified :displayName :locale :country] :required [:email :emailVerified]
    :coerce {:emailVerified :bool} :refs {:accountId "Account"}}
   {:entity "SharedFolder"    :plural "sharedfolders"    :id-prefix "dropbox_sha"
    :fields [:sharedFolderId :name :pathLower :accessType] :required [:name :pathLower]
    :coerce {} :refs {:sharedFolderId "SharedFolder"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"dropbox.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dropbox-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dropbox-compat/ready)
